# scripts/submit_and_collect_concurrent_v3.py
"""
Submit & collect OpenAI Batch API jobs for RS/RC annotation requests (v3).

Layout (requests produced by prepare_requests_v3.py):

  batch/Requests_v3/submissions/day=YYYY-MM-DD/part-00000.jsonl
  batch/Requests_v3/comments/day=YYYY-MM-DD/part-00000.jsonl

Results:

  batch/Results_v3/submissions/day=YYYY-MM-DD/part-00000.jsonl
  batch/Results_v3/comments/day=YYYY-MM-DD/part-00000.jsonl

Notes:
- Generic: caller passes --requests_root / --results_root.
- Uses /v1/responses endpoint for Batch.
"""
...
def result_paths(results_root: Path, kind: str, day: dt.date, req_part_path: Path) -> Tuple[Path, Path]:
    """
    Mirror the requests path under results_root.
...
def main():
    ...
    ap.add_argument("--requests_root", required=True, help="e.g. batch/Requests_v3")
    ap.add_argument("--results_root", required=True, help="e.g. batch/Results_v3")
    ...
    ap.add_argument("--desc", default="reddit_llm_v3")
    ...
    # Build queue (skip any that already have results or errors)
    ...
    client = OpenAI()
    in_flight: Dict[str, Tuple[str, dt.date, Path, Path, Path]] = {}
                    ...
                    if err_id:
                        ...
                        try:
                            if hasattr(b, "model_dump"):
                                dump_debug(b.model_dump(), debug_root, batch_id)
                            else:
    ...
    print("All v3 batches processed.")
