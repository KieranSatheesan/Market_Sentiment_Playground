...
"""
Fetch OHLCV from Yahoo Finance for symbols present in reddit_daily,
compute basic market labels, and write per-day parquet partitions.

- Input:  data/derived/featuresets/reddit_daily/day=YYYY-MM-DD/*.parquet
- Output: data/derived/featuresets/prices_yahoo/day=YYYY-MM-DD/part-*.parquet

Behavior:
- Discovers symbols from reddit_daily for the requested window.
...
def _discover_symbols(reddit_daily_root: str, start: str, end: str) -> Tuple[Set[str], List[pd.Timestamp]]:
    ...
    for d in days:
        ...
        try:
            ...
            print(f"[WARN] Failed to read reddit_daily {d.date()}: {e}")
            continue
...
def _download_batch(tickers: List[str], start_inclusive: str, end_exclusive: str, max_retries=3, sleep=2.0) -> pd.DataFrame:
    ...
    if isinstance(data.columns, pd.MultiIndex):
        ...
        for t in tickers:
            ...
            tmp["symbol"] = t
            keep = ["day", "symbol"] + [c for c in ["Open", "High", "Low", "Close", "Adj Close", "Volume"] if c in tmp.columns]
...
def fetch_prices(
    ...
    if not symbols:
        print("[INFO] No symbols discovered in reddit_daily for this window.")
        return
    ...
    # Clamp to the original inclusive window [start_dt, end_dt] just in case
    ...
    prices["ret1_roll_std"] = (
        prices.groupby("symbol")["ret1"].rolling(ret_roll, min_periods=3).std().reset_index(level=0, drop=True)
    )
    prices["vol_roll_mean"] = (
        prices.groupby("symbol")["Volume"].rolling(vol_roll, min_periods=3).mean().reset_index(level=0, drop=True)
    )
    ...
    print(f"[DONE] prices_yahoo: wrote {written} day partitions (skipped existing: {len(days) - written}).")


def main():
    ap = argparse.ArgumentParser(description="Fetch Yahoo OHLCV and compute basic labels per (symbol, day).")
    ap.add_argument("--reddit-daily-root", required=True, help="Path to featuresets/reddit_daily")
    ap.add_argument("--out-root", required=True, help="Output root for featuresets/prices_yahoo")
