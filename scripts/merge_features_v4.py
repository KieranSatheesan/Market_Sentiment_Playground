...
"""
Merge v4 reddit_daily and prices_yahoo on (symbol, day) to produce a single tidy table.

Inputs:
  <reddit_root>/day=YYYY-MM-DD/*.parquet      (reddit_daily_v4)
  <prices_root>/day=YYYY-MM-DD/*.parquet      (prices_yahoo_v4)

Output:
  <out_root>/day=YYYY-MM-DD/part-*.parquet

Behavior:
- Inner-join on (symbol, day).
...
def merge_features_v4(
    ...
    print(f"[DONE] merged_v4: wrote {written} day partitions (skipped existing: {len(days) - written}).")


def main():
    ap = argparse.ArgumentParser(
        description="Merge v4 reddit_daily and prices_yahoo per day on (symbol, day)."
    )
    ap.add_argument("--reddit-root", required=True, help="Path to featuresets/reddit_daily_v4")
    ap.add_argument("--prices-root", required=True, help="Path to featuresets/prices_yahoo_v4")
    ap.add_argument("--out-root", required=True, help="Output root for featuresets/merged_v4")
