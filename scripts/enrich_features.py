...
"""
Enrich merged daily table with leakage-safe lags, rolling windows, deltas, and targets.

- Input:  data/derived/featuresets/merged/day=YYYY-MM-DD/*.parquet
- Output: data/derived/featuresets/merged_enriched/day=YYYY-MM-DD/part-*.parquet
...
- Z-scores/ranks NOT persisted (leave to modeling step).
...
def _add_rolling(g: pd.DataFrame, sum_cols: Sequence[str], mean_cols: Sequence[str], windows=(3, 7, 14, 30)) -> pd.DataFrame:
    g = g.sort_values("day").copy()
    for W in windows:
        for c in sum_cols:
            if c in g.columns:
...
def enrich_features(
    ...
    reddit_sum_cols = ("chatter_count", "engagement_sum")
    reddit_mean_cols = ("sent_mean", "sent_weighted")

    # Lags apply to:
    lag_cols = (
        "chatter_count", "unique_authors", "user_concentration",
        "sent_mean", "sent_weighted", "avg_conf", "median_conf",
        "engagement_sum", "comments_sum", "score_sum",
        "subreddit_entropy",
    )
    ...
    print(f"[DONE] merged_enriched: wrote {written} day partitions (skipped existing: {len(days) - written}).")


def main():
    ap = argparse.ArgumentParser(description="Enrich merged daily table with lags, rolls, deltas, and targets.")
    ap.add_argument("--merged-root", required=True, help="Path to featuresets/merged")
    ap.add_argument("--out-root", required=True, help="Output root for featuresets/merged_enriched")
