...
"""
Submit & collect OpenAI Batch API jobs for RS/RC annotation requests (v4).

Layout (requests produced by prepare_requests_v4.py):

  batch/Requests_v4/submissions/day=YYYY-MM-DD/part-00000.jsonl
  batch/Requests_v4/comments/day=YYYY-MM-DD/part-00000.jsonl

Results:

  batch/Results_v4/submissions/day=YYYY-MM-DD/part-00000.jsonl
  batch/Results_v4/comments/day=YYYY-MM-DD/part-00000.jsonl
"""
...
def dump_debug(json_obj: Any, debug_dir: Path, batch_id: str):
    ...
    try:
        ...
        print(f"[v4][DEBUG] Saved batch debug → {out}")
    except Exception as e:
        print(f"[v4][DEBUG] Failed to write debug JSON: {e}")
...
def main():
    ...
    print(f"[v4] Total request files to submit: {len(queue)}")
    if not queue:
        return
    ...
    while queue and len(in_flight) < args.max_in_flight:
        ...
        print(f"[v4] Submitted {req} -> batch {b.id}")
    ...
    while in_flight or queue:
        for batch_id in list(in_flight.keys()):
            ...
            if last_status.get(batch_id) != b.status:
                print(f"[v4][{batch_id}] {KIND_DIR[kind]} day={day} status={b.status}")
            ...
            if b.status in ("completed", "failed", "expired", "cancelled"):
                ...
                if out_ids:
                    ...
                    if err_id:
                        ...
                        print(f"[v4] No output. Wrote marker → {err}")
                        try:
                ...
                if queue and len(in_flight) < args.max_in_flight:
                    ...
                    print(f"[v4] Submitted {r2} -> batch {b2.id}")
